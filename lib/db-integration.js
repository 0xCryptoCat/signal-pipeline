...
async function storeSignalData(db, signal, walletDetails, avgScore) {
  ...
  try {
    ...
    results.tokenData = {
      ...
      pPeak: tokRecord.pPeak || tokRecord.p0, // Peak price since signal (for performance tracking)
      scnt: tokRecord.scnt,
    ...
    if (existingIdx >= 0) {
      ...
      indexRecord.trackedTokens.push({
        ...
        sym: signal.tokenSymbol,
        p0: signalPrice,
        pPeak: signalPrice, // Peak price since signal (for performance tracking)
        scnt: 1,
        avgScr: avgScore,
        firstSeen: Date.now(),
        lastSig: Date.now(),
      ...
      // Keep last 15 tokens (each ~280 chars, fits in 3800 limit)
      if (indexRecord.trackedTokens.length > 15) {
...
function getTokenEnhancement(db, tokenAddress) {
  ...
  return {
    ...
    walletPrefixes: tokRecord.wals || [], // For checking new vs repeat
  };
}

/**
 * Identify which wallets are new vs repeat buyers for a token
 * Returns { newWallets: [], repeatWallets: [], totalUnique: number }
 */
function categorizeWallets(db, tokenAddress, walletDetails) {
  const tokKey = Keys.token(tokenAddress);
  const tokRecord = db.get('tokens', tokKey);
  
  const existingPrefixes = new Set(tokRecord?.wals || []);
 ...
 * Uses tokenPeaks from index to determine win rate
 * 
 * @param {Object} db - TelegramDB instance
 * @param {string} walletAddress - Full wallet address
 * @returns {Object} { winRate, avgPeak, totalEntries, wins, stars }
 */
function getWalletReputation(db, walletAddress) {
  const walKey = Keys.wallet(walletAddress);
  const walRecord = db.get('wallets', walKey);
  
  if (!walRecord || !walRecord.toks || walRecord.toks.length === 0) {
    return { winRate: 0, avgPeak: 0, totalEntries: 0, wins: 0, stars: 0, isNew: true };
  }
  
  // Get tokenPeaks from index
  const indexKey = Keys.index();
  const index = db.get('index', indexKey);
  const tokenPeaks = index?.tokenPeaks || {};
  
  let wins = 0;
  let totalPeak = 0;
  let counted = 0;
  
  for (const tok of walRecord.toks) {
    const [tokenPrefix, eventTime, entryPrice, storedPeak] = tok;
    
    // Use stored peak if available, otherwise look up from index
    let peakMult = storedPeak;
    if (!peakMult && tokenPeaks[tokenPrefix]) {
      // Calculate peak relative to THIS wallet's entry price
      const tokenData = tokenPeaks[tokenPrefix];
      peakMult = tokenData.peak * (tokenData.entry / entryPrice);
    }
    
    if (peakMult && peakMult > 0) {
      counted++;
      totalPeak += peakMult;
      if (peakMult >= 1.5) wins++; // 1.5x = win
    }
  }
  
  if (counted === 0) {
    return { winRate: 0, avgPeak: 0, totalEntries: walRecord.toks.length, wins: 0, stars: 0, isNew: true };
  }
  ...
  let stars = 0;
  if (winRate >= 70 && avgPeak >= 2.0) stars = 3;
  else if (winRate >= 50 && avgPeak >= 1.5) stars = 2;
  else if (winRate >= 30) stars = 1;
  
  return {
    winRate: Math.round(winRate),
    avgPeak: round(avgPeak, 2),
    totalEntries: walRecord.toks.length,
    wins,
    stars,
    isNew: false,
  };
