...
async function storeSignalData(db, signal, walletDetails, avgScore, security) {
  ...
  try {
    ...
    if (token) {
      ...
      // Track signal history for chart plotting
      if (!token.signals) token.signals = [];
      token.signals.push({
        time: signal.eventTime,
        price: signalPrice,
        score: avgScore,
      });
      // Keep last 20 signals max
      if (token.signals.length > 20) token.signals.shift();
      
      // Log slippage (difference between OKX signal price and our processed price)
      // Note: signal.priceAtSignal is from OKX, signalPrice is what we parsed
      // This is logged but not displayed
      const okxPrice = parseFloat(signal.priceAtSignal) || 0;
      if (okxPrice > 0 && token.p0 > 0) {
        const slippage = ((okxPrice - token.p0) / token.p0) * 100;
        if (Math.abs(slippage) > 0.1) {
          console.log(`   📊 Slippage: ${slippage.toFixed(2)}% (OKX: ${okxPrice}, First: ${token.p0})`);
        }
      }
      ...
      token = {
        ...
        avgScr: avgScore,
        firstSeen: Date.now(),
        mc0: parseFloat(signal.mcapAtSignal) || null,  // Market cap at first signal
        // Signal history for chart plotting
        signals: [{
          time: signal.eventTime,
          price: signalPrice,
          score: avgScore,
        }],
        // Journey tracking (updated by update-prices.js)
        hitPeakAfterDip: false,  // Went below entry then achieved peak above entry
        dippedAfterPeak: false,  // Went above 1.25x then dipped below entry
...
function getTokenEnhancement(db, tokenAddress) {
  ...
  return {
    ...
    security: token.sec,
    // Fields needed for chart signal plotting
    firstSeen: token.firstSeen,
    lastSig: token.lastSig,
    signals: token.signals || [], // Array of {time, price} for each signal
    // Journey tracking
    hitPeakAfterDip: token.hitPeakAfterDip,  // True if went below entry then achieved peak
    dippedAfterPeak: token.dippedAfterPeak,  // True if went above 1.25x then below entry
  };
}
...
function getWalletReputation(db, walletAddress) {
  ...
  if (!wallet || !wallet.tokens || Object.keys(wallet.tokens).length === 0) {
    return { winRate: 0, avgPeak: 0, totalEntries: 0, wins: 0, stars: 0, normalizedScore: 0, isNew: true };
  }
  ...
  if (counted === 0) {
    // Calculate normalizedScore even for wallets with no tracked peaks
    const normalizedScore = calcWalletRankScore(wallet, tokenPeaks);
    return { winRate: 0, avgPeak: 0, totalEntries: Object.keys(wallet.tokens).length, wins: 0, stars: 0, normalizedScore, isNew: true };
  }
  
  const winRate = (wins / counted) * 100;
  const avgPeak = totalPeak / counted;
  
  // Calculate star rating and normalized score (0-100)
