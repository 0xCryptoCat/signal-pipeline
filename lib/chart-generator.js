...
function registerCustomFonts() {
  ...
  try {
    ...
    if (fontPath) {
      ...
      // Use a unique family name to avoid conflicts
      registerFont(fontPath, { family: 'CustomRoboto', weight: 'bold' });
      registerFont(fontPath, { family: 'CustomRoboto', weight: 'normal' });
...
export async function generateChart(chainKey, tokenSymbol, tokenLogoUrl, priceData = null, signalEntries = [], signalTimestamps = []) {
  ...
  const customPlugin = {
    ...
    beforeDraw: (chart) => {
      ...
      ctx.font = 'bold 120px "CustomRoboto", Arial, sans-serif'; 
      ...
      try {
        ...
        ctx.font = 'bold 64px "CustomRoboto", Arial, sans-serif';
