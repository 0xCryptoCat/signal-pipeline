...
const THEMES = {
  sol: {
    ...
    name: 'Solana',
    icon: 'https://web3.okx.com/cdn/wallet/logo/SOL-20220525.png'
  },
  eth: {
    color: '#C0C0C0', // Silver
    bgGradient: ['#1a1a1a', '#101010'],
    name: 'Ethereum',
    icon: '	https://web3.okx.com/cdn/wallet/logo/ETH-20220328.png'
  },
  bsc: {
    color: '#F3BA2F', // Yellow
    bgGradient: ['#1a1a1a', '#181200'],
    name: 'BSC',
    icon: '	https://web3.okx.com/cdn/web3/oklinkadmin/picture/new_bsc_chain_color.png'
  },
  base: {
    color: '#0052FF', // Blue
    bgGradient: ['#1a1a1a', '#00081a'],
    name: 'Base',
    icon: 'https://web3.okx.com/cdn/wallet/logo/base_20800_new.png'
  }
};
...
export async function generateChart(chainKey, tokenSymbol, tokenLogoUrl, priceData = null, signalEntries = [], signalTimestamps = []) {
  ...
  // registerCustomFonts(); // DISABLED: Font rendering issues on Vercel
  ...
  const customPlugin = {
    ...
    beforeDraw: (chart) => {
      ...
      // 2. Watermark (REMOVED due to font issues)
      /*
      ctx.save();
      ...
      */
      ...
      try {
        // Chain Icon
        if (chainLogoImage) {
          // Draw border for chain icon
          ctx.save();
          ctx.beginPath();
          ctx.arc(padding + iconRadius, padding + iconRadius, iconRadius + 4, 0, Math.PI * 2);
        ...
        if (tokenLogoImage) {
          // Draw border for token icon
          ctx.save();
          ctx.beginPath();
          ctx.arc(tokenIconX + iconRadius, padding + iconRadius, iconRadius + 4, 0, Math.PI * 2);
          ctx.fillStyle = theme.color;
          ctx.fill();
          ctx.restore();
        ...
        // Text: Symbol (REMOVED due to font issues)
        /*
