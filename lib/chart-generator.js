...
try {
  // Resolve relative to this file (lib/chart-generator.js -> ../fonts/Roboto-Bold.ttf)
  const fontPath = path.join(__dirname, '..', 'fonts', 'Roboto-Bold.ttf');
  
  if (fs.existsSync(fontPath)) {
    registerFont(fontPath, { family: 'Roboto', weight: 'bold' });
  } else {
    // Fallback: Try process.cwd() just in case
    const cwdPath = path.join(process.cwd(), 'fonts', 'Roboto-Bold.ttf');
    if (fs.existsSync(cwdPath)) {
      registerFont(cwdPath, { family: 'Roboto', weight: 'bold' });
    } else {
      console.warn(`Font file not found at ${fontPath} or ${cwdPath}`);
    }
  }
} catch (e) {
...
export async function generateChart(chainKey, tokenSymbol, tokenLogoUrl, priceData = null, signalEntries = []) {
  ...
  const customPlugin = {
    ...
    beforeDraw: (chart) => {
      ...
      ctx.font = 'bold 150px Roboto'; // Increased size
