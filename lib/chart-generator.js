...
try {
  ...
  for (const p of pathsToTry) {
    if (fs.existsSync(p)) {
      console.log(`Found font at ${p}, registering...`);
      
      // Register as 'Roboto' with bold weight
      registerFont(p, { family: 'Roboto', weight: 'bold' });
      
      // Register as 'Roboto' with normal weight (fallback)
      registerFont(p, { family: 'Roboto', weight: 'normal' });
      
      // Register as just 'Roboto' (no options)
      registerFont(p, { family: 'Roboto' });

      // FORCE OVERRIDE SYSTEM FONTS
      // This is crucial for Vercel/Lambda where 'sans-serif' might not exist
      registerFont(p, { family: 'sans-serif', weight: 'bold' });
      registerFont(p, { family: 'sans-serif', weight: 'normal' });
      registerFont(p, { family: 'sans-serif' });
