...
export class LeaderboardManager {
  ...
  formatChainWalletLeaderboard(chain, wallets, channelId, isPublic = false) {
    ...
    wallets.slice(0, 10).forEach((w, i) => {
      ...
      // Build wallet display (with link for private, code for public)
      let walletDisplay;
      if (isPublic) {
      ...
      // Format: 1. <code>ADDR</code> │ ROI │ Win% │ 🟢 +0.85 (N)
      msg += `<code>${rank}. ${isPublic ? walletDisplay : ''}</code>${!isPublic ? walletDisplay : ''}<code> │ ${roiSign}${roi}% │ ${winRate}% │ </code>${emoji}<code> ${scoreStr} (${entries})</code>\n`;
    });
  ...
  formatChainTokenLeaderboard(chain, tokens, channelId, isPublic = false) {
    ...
    tokens.slice(0, 10).forEach((t, i) => {
      ...
      // ATH multiplier (lowest entry to peak)
      const peakMult = t.peakMult || t.mult || 1;
      const multStr = peakMult.toFixed(1) + 'x';
      
      // Signal count with emoji
      const sigs = t.scnt || 1;
      
      // Trending score with color emoji
      const trendScore = t.trendScore || 0;
      const scoreEmoji = trendScore >= 1.5 ? '🔵' : trendScore >= 0.5 ? '🟢' : trendScore >= -0.5 ? '⚪️' : trendScore >= -1.5 ? '🟠' : '🔴';
      
      // Format: 1. TOKEN │ 2.5x ATH │ 📊 5 │ 🟢 +1.23
      msg += `<code>${rank}. </code>${tokenDisplay}<code> │ ${multStr} ATH │ 📊 ${sigs} │ </code>${scoreEmoji}<code> ${trendScore >= 0 ? '+' : ''}${trendScore.toFixed(2)}</code>\n`;
    });
