...
export class LeaderboardManager {
   ...
   */
  formatSummaryMessage(channelId, isPublic = false, topTokens = [], hallOfFameMsgId = null) {
   ...
   * @param {Array} topTokens - Top tokens across all chains
   */
  async updateSummaryMessage(channelId, isPublic = false, topTokens = [], hallOfFameMsgId = null) {
    const key = isPublic ? 'public' : 'private';
    const messageId = this.config?.summaries?.[key];
    const text = this.formatSummaryMessage(channelId, isPublic, topTokens, hallOfFameMsgId);
