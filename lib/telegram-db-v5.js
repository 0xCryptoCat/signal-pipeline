...
export class LeaderboardManager {
  ...
  async updateAll(allChainDBs) {
    ...
    // Load existing message IDs (or use empty)
    let ids;
    try {
    ...
    try {
      ...
      console.log(`   ⚠️ Private channel update failed: ${err.message}`);
    }
    ...
    try {
      ...
      console.log(`   ⚠️ Public channel update failed: ${err.message}`);
    }
    
    // Save updated message IDs (ignore errors)
    try {
