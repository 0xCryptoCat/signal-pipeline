...
export class LeaderboardManager {
   ...
   */
  formatSummaryMessage(channelId, isPublic = false, topTokens = []) {
    const leaderboards = this.config?.leaderboards || {};
    const chains = ['sol', 'eth', 'bsc', 'base'];
    
    let msg = `🏆 <b>Top 25 Gainers (7D)</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━\n\n`;
    
    if (topTokens.length === 0) {
      msg += `<i>No gainers tracked yet</i>\n\n`;
    } else {
      // Show Top 25 cross-chain gainers
      topTokens.slice(0, 25).forEach((t, i) => {
        const rank = i + 1;
        const sym = t.sym || '???';
        const mult = t.mult || 1;
        const multStr = mult >= 10 ? mult.toFixed(0) + 'x' : mult.toFixed(1) + 'x';
    ...
    msg += `\n<b>🎯 Signal Scoring:</b>\n`;
    msg += `🔵 Excellent  🟢 Good  ⚪️ Neutral  🟠 Weak  🔴 Poor\n`;
    
    if (!isPublic) {
      msg += `\n━━━━━━━━━━━━━━━━━━\n`;
      msg += `<i>🔗 Tap token to view signal</i>`;
    }
   ...
   * @param {Array} topTokens - Top tokens across all chains
   */
  async updateSummaryMessage(channelId, isPublic = false, topTokens = []) {
    const key = isPublic ? 'public' : 'private';
    const messageId = this.config?.summaries?.[key];
    const text = this.formatSummaryMessage(channelId, isPublic, topTokens);
  ...
  async updateAll(allChainDBs) {
    ...
    // Sort all tokens by multiplier and take top 25
