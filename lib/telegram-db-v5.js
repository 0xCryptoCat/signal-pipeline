...
export class LeaderboardManager {
  ...
  formatChainWalletLeaderboard(chain, wallets, channelId, isPublic = false) {
    ...
    wallets.slice(0, 10).forEach((w, i) => {
      ...
      msg += `${rank}. ${walletDisplay} │ <code>${roiSign}${roi}% │ ${winRate}%</code> │ ${emoji} <code>${scoreStr}</code> <i>(${entries})</i>\n`;
    });
  ...
  formatChainTokenLeaderboard(chain, tokens, channelId, isPublic = false) {
    ...
    tokens.slice(0, 10).forEach((t, i) => {
      ...
      // Token display with message link
      ...
      msg += `${rank}. ${tokenDisplay} │ <code>${multStr} │ 🚨${sigs}</code> │ ${trendEmoji}\n`;
    });
  ...
  formatSummaryMessage(channelId, isPublic = false) {
    ...
    let msg = `📊 <b>Alphalert Stats & Leaderboards</b>\n\n`;
    
    if (isPublic) {
      msg += `<i>Track smart money signals across multiple chains. See which wallets and tokens are trending.</i>\n\n`;
    } else {
      msg += `<i>Your private smart money signal feed. Full wallet addresses, explorer links, and detailed analytics.</i>\n\n`;
    }
    ...
    msg += `\n<b>🎯 Signal Scoring:</b>\n`;
    msg += `🔵 Excellent (+1.5 to +2)\n`;
    msg += `🟢 Good (+0.5 to +1.5)\n`;
    msg += `⚪️ Neutral (-0.5 to +0.5)\n`;
    msg += `🟠 Weak (-1.5 to -0.5)\n`;
    msg += `🔴 Poor (-2 to -1.5)\n`;
    
    if (isPublic) {
      msg += `\n━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
      msg += `🔓 <b>Want full wallet addresses & explorer links?</b>\n`;
      msg += `Join our private channel for premium access!\n`;
    } else {
      msg += `\n━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
      msg += `<i>Updated every 30 minutes</i>`;
    }
