...
export function calcWalletRankScore(wallet, tokenPeaks = {}) {
  // 1. Average Entry Score (Quality)
  // wallet.avgScr IS the average (Sum of scores / Total entries)
  const avgEntryScore = wallet.avgScr || 0;
  const qualityScore = (avgEntryScore + 2) / 4; // Normalized 0-1
  
  // 2. Entry Count (Volume)
  // Diminishing returns to prevent spamming
  const entryCountFactor = Math.sqrt(Math.min(wallet.scnt || 0, 50)) / Math.sqrt(50);
  
  // 3. Win Rate (Performance)
  // Based on actual price performance of tokens signaled
  let wins = 0, total = 0;
  ...
  // Final score weights:
  // Quality (Avg Score): 40%
  // Volume (Count): 20%
  // Performance (Win Rate): 25%
  // Consistency: 15%
  const score = (
    qualityScore * 0.40 +
...
export class TelegramDBv5 {
   ...
   * Sorted by current gains (multiplier)
   * Filter: Must be above entry price (>= 1.0x) and not archived/rugged
   */
  getTopTokens(n = 15) {
    const tokens = Object.entries(this.db?.tokens || {})
      .map(([addr, token]) => ({
        addr,
        ...token,
        // Use current multiplier for ranking
        rankScore: token.mult || 1,
      }))
      // Filter: Not rugged, not archived, and currently in profit
      // CHANGED: Removed slice(0, n) to return ALL tokens for total calculation if needed
      // But for leaderboard display we usually only need top N.
      // The caller can slice if they want, but let's keep it consistent with method signature
      // Actually, user asked to "ensure the token leaderboards display all tokens we tracked for gains"
      // This might mean they want the total stats to reflect ALL gains, not just top 10.
      // But the message format only shows top 10.
      // Let's return ALL qualifying tokens here, and let the formatter slice for display.
      .filter(t => !t.rugged && !t.archived && t.mult >= 1.0)
      .sort((a, b) => b.rankScore - a.rankScore);
    
    return tokens; // Return all, let formatter handle slicing
  }
...
export class LeaderboardManager {
  ...
  formatChainTokenLeaderboard(chain, tokens, channelId, isPublic = false) {
    ...
    let msg = `${chainInfo.emoji} <b>${chainInfo.name} Top Performing Signals</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    
    if (tokens.length === 0) {
      msg += `<i>No active signals yet</i>\n`;
      return msg;
    }
    
    // Calculate total stats for ALL tokens (not just top 10)
    let totalPctGain = 0;
    let totalMultiplier = 0;
    
    for (const t of tokens) {
      const mult = t.mult || 1;
      totalPctGain += (mult - 1) * 100;
      totalMultiplier += mult;
    }
    
    const avgMultiplier = totalMultiplier / tokens.length;
    ...
    const totalPctStr = `+${formatCompact(totalPctGain)}%`;
    const avgMultStr = `(${avgMultiplier.toFixed(1)}x)`;
    
    msg += `<b>Total Gains (${tokens.length}): ${totalPctStr} ${avgMultStr}</b>\n\n`;
    ...
    tokens.slice(0, 10).forEach((t, i) => {
      ...
      // Format: <code>1. │ 18.5x │ 🚨 2 │ </code>TOKEN
      msg += `<code>${rank}. │ ${multStr} │ 🚨 ${sigs} │ </code>${tokenDisplay}\n`;
    });
  ...
  async updateAll(allChainDBs) {
    ...
    for (const chain of chains) {
      ...
      // TEMPORARILY DISABLED: Public channel leaderboards
      // TODO: Re-enable when premium channel is ready
      // try {
