...
export class TelegramDBv5 {
  ...
  getHallOfFame(n = 25) {
    ...
    const tokens = Object.entries(this.db?.tokens || {})
      ...
      // Filter: Older than 7D, achieved at least 2x peak (notable gains)
      .filter(t => !t.rugged && t.peakMult >= 2.0 && (t.lastSig || t.firstSeen || 0) <= cutoff7d)
      .sort((a, b) => b.rankScore - a.rankScore);
...
export class LeaderboardManager {
  ...
  formatChainTokenLeaderboard(chain, tokens7d, tokens1d, channelId, isPublic = false) {
    ...
    // Helper: Calculate sum of gains (mult - 1 for each, so 1.5x adds 0.5)
    const calcGainSum = (tokens) => tokens.reduce((sum, t) => sum + Math.max(0, (t.peakMult || 1) - 1), 0);
  ...
  formatSummaryMessage(channelId, isPublic = false, topTokens = []) {
    ...
    // Calculate sum of gains (mult - 1 for each)
    const displayed = topTokens.slice(0, 25);
    const gainSum = displayed.reduce((sum, t) => sum + Math.max(0, (t.peakMult || 1) - 1), 0);
    const sumStr = gainSum >= 100 ? gainSum.toFixed(0) : gainSum.toFixed(1);
    
    let msg = `🏆 <b>Top 25 Gainers - 7D (${sumStr}x)</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━\n\n`;
  ...
  formatHallOfFame(channelId, isPublic = false, hallOfFame = []) {
    ...
    msg += `━━━━━━━━━━━━━━━━━━\n`;
    msg += `<i>Best performers past 7D (2x+ peak)</i>\n\n`;
